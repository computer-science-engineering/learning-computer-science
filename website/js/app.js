...
var getJson = new Promise(function (resolve, reject) {
  'use strict';
  //fetch('https://raw.githubusercontent.com/manastalukdar/manastalukdar.github.io/master/problems_list.json', {mode: 'cors'})// uncomment for debugging
...
function stringifyNestedJsonArrays(json_data) {
  ...
  return new Promise(function (resolve, reject) {
    temp_json_data.forEach(function (element) {
      var temp;
      if (
        element.hasOwnProperty(global_problem_companies_string) &&
        element[global_problem_companies_string] !== ''
      ) {
      ...
      if (
        element.hasOwnProperty(global_problem_categories_string) &&
        element[global_problem_categories_string] !== ''
      ) {
      ...
      if (
        element.hasOwnProperty(global_problem_tags_string) &&
        element[global_problem_tags_string] !== ''
      ) {
      ...
      if (
        element.hasOwnProperty(global_problem_languages_string) &&
        element[global_problem_languages_string] !== ''
      ) {
...
function createTable(json_data) {
  ...
  return new Promise(function (resolve, reject) {
    var columnJson = $.map(
      Object.getOwnPropertyNames(json_data[0]),
      function (column) {
        if (column === global_problem_id_string) {
          ...
          column === global_problem_name_string ||
          column === global_problem_companies_string ||
          column === global_problem_tags_string
        ) {
    ...
    stringifyNestedJsonArrays(json_data)
      .then((nested_arrays_removed_json) =>
        createFooTable(columnJson, nested_arrays_removed_json)
      )
...
function adaptJsonData(json_data) {
  ...
  return new Promise(function (resolve, reject) {
    Array.prototype.forEach.call(json_data, (element) => {
      if (
        element.hasOwnProperty(global_problem_name_string) &&
        element.hasOwnProperty(global_problem_link_string)
      ) {
        var problem_name = element.Name;
        var link = element.Link;
        element.Name =
          '<a href="' + link + '" target="_blank">' + problem_name + '</a>';
        delete element.Link;
      }
  ...
  return new Promise(function (resolve, reject) {
    ...
    $('#input-origin').multiselect({
      ...
      templates: {
        ...
        button:
          '<button type="button" class="multiselect dropdown-toggle" data-toggle="dropdown" data-flip="false"><span class="multiselect-selected-text"></span> <b class="caret"></b></button>',
        filter:
          '<li class="multiselect-item filter"><div class="input-group m-0"><input class="form-control multiselect-search" type="text"></div></li>',
        filterClearBtn:
          '<span class="input-group-btn"><button type="button" class="btn btn-secondary multiselect-clear-filter">&times;</button></span>',
      },
      onChange: function () {
    ...
    $('#input-companies').multiselect({
      ...
      onChange: function () {
        ...
        if (selected.length === 0) {
          $('#filter-query-companies').html(
            global_filter_query_companies_default
          );
        } else {
          var arrayAsString = selected.join(' OR ');
          var text = '(' + arrayAsString + ')';
    ...
    $('#input-categories').multiselect({
      ...
      onChange: function () {
        ...
        if (selected.length === 0) {
          $('#filter-query-categories').html(
            global_filter_query_categories_default
          );
        } else {
          var arrayAsString = selected.join(' OR ');
          var text = '(' + arrayAsString + ')';
    ...
    $('#input-languages').multiselect({
      ...
      onChange: function () {
        ...
        if (selected.length === 0) {
          $('#filter-query-languages').html(
            global_filter_query_languages_default
          );
        } else {
          var arrayAsString = selected.join(' OR ');
          var text = '(' + arrayAsString + ')';
    ...
    $('#input-and-or-origin-companies').multiselect({
      ...
      onChange: function () {
        ...
        if (selected.length === 0) {
          $('#filter-query-and-or-origin-companies').html(
            global_filter_query_and_or_default
          );
        } else {
    ...
    $('#input-and-or-companies-categories').multiselect({
      ...
      onChange: function () {
        ...
        if (selected.length === 0) {
          $('#filter-query-and-or-companies-categories').html(
            global_filter_query_and_or_default
          );
        } else {
          var arrayAsString = selected.join('');
    ...
    $('#input-and-or-categories-tags').multiselect({
      ...
      onChange: function () {
        ...
        if (selected.length === 0) {
          $('#filter-query-and-or-categories-tags').html(
            global_filter_query_and_or_default
          );
        } else {
          var arrayAsString = selected.join('');
    ...
    $('#input-and-or-tags-languages').multiselect({
      ...
      onChange: function () {
        ...
        if (selected.length === 0) {
          $('#filter-query-and-or-tags-languages').html(
            global_filter_query_and_or_default
          );
        } else {
          var arrayAsString = selected.join('');
...
function getFilteredJson() {
  ...
  return new Promise(function (resolve, reject) {
    ...
    temp = $('#input-and-or-origin-companies option:selected').map(function (
      a,
      item
    ) {
      return item.value;
    ...
    temp = $('#input-and-or-companies-categories option:selected').map(
      function (a, item) {
        return item.value;
      }
    );
    var and_or_companies_categories = _.join(temp, '');
    temp = $('#input-and-or-categories-tags option:selected').map(function (
      a,
      item
    ) {
      return item.value;
    ...
    temp = $('#input-and-or-tags-languages option:selected').map(function (
      a,
      item
    ) {
      return item.value;
